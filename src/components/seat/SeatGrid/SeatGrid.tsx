...
const SeatSelection: React.FC<SeatMapProps> = ({ seats, onSeatSelect }) => {
  const renderSeatMap = () => {
    ...
    return Object.keys(seatsByRow).map(rowNum => {
      ...
      const leftSeats = rowSeats.filter(
        seat => seat.id.endsWith("A") || seat.id.endsWith("B") || seat.id.endsWith("C")
      );

      const rightSeats = rowSeats.filter(
        seat => seat.id.endsWith("D") || seat.id.endsWith("E") || seat.id.endsWith("F")
      );
      ...
      const createSeatElement = (seat: SeatInfo) => {
        ...
        return (
          <div key={seat.id} className={seatClassName} onClick={() => onSeatSelect(seat.id)}>
            {seat.id}
          </div>
        );
      };
  ...
  return <div className={styles.seatGrid}>{renderSeatMap()}</div>;
};
